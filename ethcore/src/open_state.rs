...
use vm::EnvInfo;
use error::Error;
use open_executive::{Executive, TransactOptions};
...
pub use state::{self, Account, Backend, Substate, CleanupMode};
...
impl<B: Backend> State<B> {
        ...
        pub fn insert_cache(&self, address: &Address, account: AccountEntry) {
        ...
        /// Remove an existing account, and return it.
        pub fn drop_account(&mut self, account: &Address) -> AccountEntry {
            self.cache.borrow_mut().remove(account).unwrap()
        }
	...
	pub fn commit_external(&mut self, db: &mut StateDB, root: &mut H256) -> Result<(), Error> {
		...
		for (address, ref mut a) in accounts.iter_mut().filter(|&(_, ref a)| a.is_dirty()) {
			if let Some(ref mut account) = a.account {
				let addr_hash = account.address_hash(address);
				{
					let mut account_db = self.factories.accountdb.create(db.as_hash_db_mut(), addr_hash);
					account.commit_storage(&self.factories.trie, account_db.as_hash_db_mut())?;
					account.commit_code(account_db.as_hash_db_mut());
				}
				if !account.is_empty() {
